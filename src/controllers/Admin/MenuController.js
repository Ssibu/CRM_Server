...
const validateMenuData = (data, isUpdate = false) => {
  ...
  } else if (data.en_title.trim().length > 35) { // NEW
  ...
  } else if (data.od_title.trim().length > 35) { // NEW
...
export const getAllMenusWithoutPagination = async (req, res) => {
  try {
    const menus = await Menu.findAll({
      // You can add an order if you want
      order: [['display_order', 'ASC']],
    });
    // This returns the full array of menus, which is what the dropdown needs
    res.status(200).json(menus);
  } catch (error) {
    console.error("Error fetching all menus for dropdown:", error);
    res.status(500).json({ message: "Error fetching menus for dropdown." });
  }
};
...
export const getAllMenus = async (req, res) => {
  try {
    ...
    const allowedSortColumns = ["en_title", "od_title", "status", "createdAt", "display_order"];
...
export const createMenu = async (req, res) => {
  let transaction;
  try {
    const { en_title, od_title, en_description, od_description, link, status, meta_title, meta_keyword, meta_description } = req.body;

    // Validate required fields
    const validation = validateMenuData({ en_title, od_title, image_url: req.file ? 'exists' : '' });
    ...
    if (existingMenu) {
      ...
      if (existingMenu.en_title === en_title.trim()) {
...
export const updateMenu = async (req, res) => {
  ...
  try {
    ...
    const { en_title, od_title, en_description, od_description, link, status, remove_image,      meta_title,
      meta_keyword,
      meta_description } = req.body;
    // Validate input
    const validation = validateMenuData({ 
      en_title, 
      od_title, 
      image_url: menu.image_url || (req.file ? 'exists' : ''), 
      isUpdate: true ,
    ...
    //  Check duplicate English/Odia titles
    if (en_title || od_title) {
      const existingMenu = await Menu.findOne({
        where: {
          [Op.or]: [
            en_title ? { en_title: en_title.trim() } : null,
            od_title ? { od_title: od_title.trim() } : null,
          ].filter(Boolean),
          id: { [Op.ne]: id } // Exclude current menu
        }
      });
      ...
      if (existingMenu) {
        ...
        let errorMessage = "A menu with the same ";
        if (en_title && existingMenu.en_title === en_title.trim()) {
          errorMessage += "English title already exists";
        } else {
          errorMessage += "Odia title already exists";
        }
    ...
    if (en_title) {
      ...
      if (existingSlug) {
        ...
        return res.status(409).json({
          message: "A menu with a similar slug already exists. Please choose a different title."
        });
      }
      menu.slug = slug;
    }
    ...
    // const updatedData = {
    //   en_title: en_title ? en_title.trim() : menu.en_title,
    //   od_title: od_title ? od_title.trim() : menu.od_title,
    //   en_description: en_description !== undefined ? en_description.trim() : menu.en_description,
    //   od_description: od_description !== undefined ? od_description.trim() : menu.od_description,
    //   link: link !== undefined ? link.trim() : menu.link,
    //   status: status || menu.status,
    //    meta_title:meta_title !== undefined ? meta_title.trim() : subSubMenu.meta_title,
    //   meta_keyword: meta_keyword !== undefined ? meta_keyword.trim()  : subSubMenu.meta_keyword,
    //   meta_description:meta_description !== undefined ? meta_description.trim() : menu.meta_description,
    // };
    // in updateMenu function
const updatedData = {
  en_title: en_title ? en_title.trim() : menu.en_title,
  od_title: od_title ? od_title.trim() : menu.od_title,
  en_description: en_description !== undefined ? en_description.trim() : menu.en_description,
  ...
  link: link !== undefined ? link.trim() : menu.link,
  status: status || menu.status,
  meta_title: meta_title !== undefined ? meta_title.trim() : menu.meta_title, // Corrected
  meta_keyword: meta_keyword !== undefined ? meta_keyword.trim() : menu.meta_keyword, // Corrected
  meta_description: meta_description !== undefined ? meta_description.trim() : menu.meta_description, // Was already correct
};
    ...
    await log({
      req,
      action: "UPDATE",
      page_name: "UPDATE MENU",
      target: menu.en_title,
    });
