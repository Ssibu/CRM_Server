...
export const addNotice = async (req, res) => {
  try {
    ...
    if (!en_title || !date) {
      return res
        .status(400)
        .json({ message: "English title and date are required." });
    }

    if (!req.file) {
      return res
        .status(400)
        .json({ message: "Notice document is required." });
    }
    ...
    if (od_title && od_title.length > 100) {
      return res.status(400).json({
        message: "Odia title must not exceed 100 characters.",
      });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Notice with this title already exists." });
    }
...
export const updateNotice = async (req, res) => {
  try {
    ...
    let { en_title, od_title, date, is_active, removeDoc } = req.body;

    // Normalize input titles
    en_title = normalizeTitle(en_title);
    od_title = normalizeTitle(od_title);

    // --- Length validation ---
    if (en_title && (en_title.length < 1 || en_title.length > 100)) {
      return res.status(400).json({
        message: "English title must be between 1 and 100 characters.",
      });
    }
    ...
    const oldFilePath = notice.doc
      ? path.join('public/uploads/notices', notice.doc)
      : null;
    ...
    notice.is_active =
      is_active !== undefined ? is_active : notice.is_active;
