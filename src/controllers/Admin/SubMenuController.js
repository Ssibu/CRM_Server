...
export const createSubMenu = async (req, res) => {
  let transaction;
  try {
    const { menuId, en_title, od_title, en_description, od_description, link, status, meta_title, meta_keyword, meta_description  } = req.body;
...
export const updateSubMenu = async (req, res) => {
  ...
  try {
    ...
    // ✅ Duplicate English / Odia title check
    ...
    const updatedData = {
      ...
      status: status !== undefined ? status : subMenu.status,
            meta_title:
        meta_title !== undefined ? meta_title.trim() : SubMenu.meta_title,
      meta_keyword:
        meta_keyword !== undefined
          ? meta_keyword.trim()
          : SubMenu.meta_keyword,
      meta_description:
        meta_description !== undefined
          ? meta_description.trim()
          : SubMenu.meta_description,
    };

    if (en_title) {
      const slug = generateSlug(en_title);
      const existingSlug = await SubMenu.findOne({
      ...
      if (existingSlug) {
        ...
        return res.status(409).json({
          message:
            "A sub menu with a similar slug already exists. Please choose a different title.",
        });
      }
    ...
    await log({
      req,
      action: "UPDATE",
      page_name: "UPDATE SUB-MENU",
      target: subMenu.en_title,
    });
