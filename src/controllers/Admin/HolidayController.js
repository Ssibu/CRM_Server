...
export const listHolidays = async (req, res) => {
  try {
    ...
    const allowedSortColumns = ["holiday_date", "name", "type", "is_active", "createdAt"];
    ...
    const whereClause = { is_delete: false, ...(search && ({ name: { [Op.like]: `%${search}%` } } , { holiday_date: { [Op.like]: `%${formattedDate}%` } } )) };
    ...
    try {
        ...
        await holiday.save();
            await log({
              ...
              action: U,
              page_name: EDIT,
              target: holiday.name || holiday.id
            });
    ...
    try {
        ...
        await holiday.save();
            await log({
              ...
              action: D,
              page_name: LIST,
              target: holiday.name || req.params.id
            });
