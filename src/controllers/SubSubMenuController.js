...
//     res.status(500).json({ message: "Error fetching sub-submenus. Please try again later.", error: error.message });
//   }
// };
...
export const updateSubSubMenu = async (req, res) => {
  ...
  try {
    ...
    // Check if parent submenu exists (if subMenuId is being updated)
    ...
    // Check for duplicate titles (excluding current sub-submenu)
    if (title_en || title_od) {
      ...
      const existingSubSubMenu = await SubSubMenu.findOne({ where: whereCondition, transaction });
    ...
    const updatedData = {
      subMenuId: subMenuId !== undefined ? subMenuId : subSubMenu.subMenuId,
      title_en: title_en !== undefined ? title_en.trim() : subSubMenu.title_en,
      title_od: title_od !== undefined ? title_od.trim() : subSubMenu.title_od,
      description_en: description_en !== undefined ? description_en.trim() : subSubMenu.description_en,
      description_od: description_od !== undefined ? description_od.trim() : subSubMenu.description_od,
      ...
      meta_description: meta_description !== undefined ? meta_description.trim() : subSubMenu.meta_description
    };
    ...
    res.status(500).json({ message: "Failed to update sub-submenu. Please try again later.", error: error.message });
  }
};
