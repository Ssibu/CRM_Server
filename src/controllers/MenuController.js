...
export const updateMenu = async (req, res) => {
  ...
  try {
    ...
    const validation = validateMenuData({ 
      title_en, 
      title_od, 
      image_url: req.file ? 'exists' : menu.image_url,
      status 
    }, true);
    ...
    // Check for duplicate titles (excluding current menu)
    if (title_en || title_od) {
      ...
      const existingMenu = await Menu.findOne({ where: whereCondition });
      ...
      if (existingMenu) {
        ...
        let errorMessage = "Another menu with the same ";
        if (existingMenu.title_en === title_en.trim()) {
          errorMessage += "English title already exists";
        } else {
          errorMessage += "Odia title already exists";
    ...
    res.status(500).json({ message: "Failed to update menu. Please try again later.", error: error.message });
  }
};
