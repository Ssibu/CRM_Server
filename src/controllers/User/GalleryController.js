...
export const getAllCategories = async (req, res) => {
  try {
    ...
    const categoryType = req.query.type || null;
  ...
  try {
    ...
    const { count, rows } = await PhotoGallery.findAndCountAll({
      ...
      include: [{
        ...
        as: 'category',
        attributes: [],
        // FIX: Also ensure the category's status is `true`.
        where: { status: true },
        required: true,
      }],
      attributes: {
...
export const getAllVideos = async (req, res) => {
  try {
    ...
    const { count, rows } = await VideoGallery.findAndCountAll({
      ...
      attributes: {
        include: [[VideoGallery.sequelize.col('category.en_category'), 'category_name']]
      },
      order,
      limit,
      offset,
...
export const getAllPhotosWithoutPagination = async (req, res) => {
  try {
    ...
    const whereClause = {
      // FIX: Changed from string "Active" to boolean `true` to match the BOOLEAN schema.
      status: true,
      ...(category_id && { category_id }),
      ...(search && {
        [Op.or]: [
    ...
    console.error('Get Photos Without Pagination Error:', error);
...
export const getAllVideosWithoutPagination = async (req, res) => {
  try {
    ...
    console.error('Get All Videos Without Pagination Error:', error);
